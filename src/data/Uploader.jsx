...
async function deleteGuests() {
  ...
  if (error) console.log('Error deleting guests:', error.message);
}
...
async function createGuests() {
  console.log('Inserting guests:', guests); // Debug
  const { data, error } = await supabase.from('guests').insert(guests);
  if (error) {
    console.error('Error creating guests:', error.message, error.details);
  } else {
...
async function createBookings() {
  const { data: guestsIds } = await supabase
    ...
    .select('id')
    .order('id');
  const allGuestIds = guestsIds.map((guest) => guest.id); // Sửa từ cabin thành guest
